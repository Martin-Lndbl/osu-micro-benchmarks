...
void run_acc_with_flush (int rank, WINDOW type)
{
    ...
    for (size = 0; size <= MAX_SIZE; size = (size ? size * 2 : 1)) {
        ...
        if(rank == 0) {
            MPI_CHECK(PMPI_Win_lock(MPI_LOCK_SHARED, 1, 0, win));
            for (i = 0; i < options.skip + options.loop; i++) {
                if (i == options.skip) {
                    t_start = PMPI_Wtime ();
                }
                MPI_CHECK(PMPI_Accumulate(sbuf, size, MPI_CHAR, 1, disp, size, MPI_CHAR, MPI_SUM, win));
    ...
    for (size = 0; size <= MAX_SIZE; size = (size ? size * 2 : 1)) {
        ...
        if(rank == 0) {
            ...
            for (i = 0; i < options.skip + options.loop; i++) {
                MPI_CHECK(PMPI_Win_fence(0, win));
                MPI_CHECK(PMPI_Win_fence(0, win));
                MPI_CHECK(PMPI_Accumulate(sbuf, size, MPI_CHAR, 0, disp, size, MPI_CHAR, MPI_SUM, win));
                MPI_CHECK(PMPI_Win_fence(0, win));
            }
        }
