    ...
    for(size = 0; size <= MAX_MSG_SIZE; size = (size ? size * 2 : 1)) {
        ...
        if(myid == 0) {
            for(i = 0; i < options.loop + options.skip; i++) {
                if(i == options.skip) t_start = PMPI_Wtime();

                PMPI_Send(s_buf, size, MPI_CHAR, 1, 1, MPI_COMM_WORLD);
                PMPI_Recv(r_buf, size, MPI_CHAR, 1, 1, MPI_COMM_WORLD, &reqstat);
            }
            ...
            for(i = 0; i < options.loop + options.skip; i++) {
                PMPI_Recv(r_buf, size, MPI_CHAR, 0, 1, MPI_COMM_WORLD, &reqstat);
                PMPI_Send(s_buf, size, MPI_CHAR, 0, 1, MPI_COMM_WORLD);
            }
        }
